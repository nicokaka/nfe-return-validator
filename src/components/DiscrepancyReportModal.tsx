...
export const DiscrepancyReportModal: React.FC<DiscrepancyReportModalProps> = ({ result, onClose }) => {
  ...
  const handleDownloadCsv = () => {
    const csvRows: string[] = [
      'Item;Status;Produto NFD;NCM;Categoria;ANVISA;EAN;CFOP NFD;CFOP Entrada Piramide;Qtd Devolvida;Preco NFD;Preco NFO;Desc NFD;Desc Esperado NFO;Desc Proporcional;Credito PIS INFAN;Credito COFINS INFAN;CBS;IBS;Lote NFD;Lote NFO;Almoxarifado Piramide;Inconsistencias',
    ];
    ...
    result.itemComparisons.forEach(c => {
      ...
      const cfopNfd = nfdItem.cfop || '';
      const cfopEntrada = result.ndoSuggestion?.cfop || '2.202';
      ...
      const credPis = (c.pisCofinsCreditAudit?.vPisCredit || 0).toFixed(2);
      const credCofins = (c.pisCofinsCreditAudit?.vCofinsCredit || 0).toFixed(2);
      const vCbs = (c.ibsCbsAudit?.vCbs || 0).toFixed(2);
      const vIbs = (c.ibsCbsAudit?.vIbs || 0).toFixed(2);
      ...
      csvRows.push(
        `${nfdItem.nItem};"${status}";"${nfdItem.xProd.replace(/"/g, '""')}";"${ncm}";"${categoria}";"${anvisa}";"${nfdItem.cEAN}";"${cfopNfd}";"${cfopEntrada}";${nfdItem.qCom};${nfdItem.vUnCom.toFixed(2)};${nfoItem ? nfoItem.vUnCom.toFixed(2) : 0};${descNfd};${descEsperado};"${descPropOk}";${credPis};${credCofins};${vCbs};${vIbs};"${lotesNfd}";"${lotesNfo}";"${almox}";"${issuesStr}"`
      );
    });
